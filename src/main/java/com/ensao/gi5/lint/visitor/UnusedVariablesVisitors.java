...
public class UnusedVariablesVisitors extends VoidVisitorAdapter<List<VariableWrapper>> {
	...
	public void visit(MethodDeclaration n, List<VariableWrapper> arg) {
		n.getBody().ifPresent(b -> b.getStatements().forEach(st -> {
			...
			if (vr != null) {
				VariableWrapper w = new VariableWrapper(vr, n.getNameAsString(), resolveClassName(n),
						variableCount(b, vr), st.getBegin().map(p -> p.line).orElse(-1));
				arg.add(w);
			}
		}));
		super.visit(n, arg);

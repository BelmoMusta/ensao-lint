package com.ensao.gi5.rules;

import static org.junit.jupiter.api.Assertions.assertEquals;

import java.io.File;

import org.junit.jupiter.api.Test;

import com.ensao.gi5.lint.rules.UnusedPrivateMethodsRule;
import com.ensao.gi5.test.constantes.Paths;
import com.ensao.gi5.test.utils.Utils;

public class UnusedPrivateMethodsRuleTest {

	@Test
	public void testUnusedPrivateMethodsRule() {
		int numViolations = Utils.getRuleViolationsNumber(new UnusedPrivateMethodsRule(),
				new File(Paths.UNUSED_PRIVATE_METHODS_FILE_PATH));
		assertEquals(2, numViolations);
	}
}
